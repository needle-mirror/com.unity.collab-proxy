...
.factory('editorCollabNative', ["$q", "$rootScope", "$timeout", "unityService", "cloudcoreUi", "unityConnectService", "unityCloudPanelService", "unityProjectService", "notifications", "StreamEvent", "utAlert", function ($q, $rootScope, $timeout, unityService, cloudcoreUi, unityConnectService, unityCloudPanelService, unityProjectService, notifications, StreamEvent, utAlert){
    ...
    var GET_CHANGES_FINISHED = "getChangesFinished";
    var GET_CHANGES_STARTED = "getChangesStarted";
    var UPDATE_CACHED_CHANGES_FINISHED = "updateCachedChangesFinished";
    var lastConflictsHash = 0;
    var currState;
    var isGettingChanges = false;
    ...
    service.onGetChangesFinish = function (callback) {
        var handle = $rootScope.$on(GET_CHANGES_FINISHED, function () {
            callback();
            handle();
        });
    };

    service.onUpdateCachedChangesFinished = function (callback) {
        var handle = $rootScope.$on(UPDATE_CACHED_CHANGES_FINISHED, function (changesAvailable) {
            callback(changesAvailable);
            handle();
        });
    };

    $rootScope.OnGetChangesStart = function () {
        $rootScope.$broadcast(GET_CHANGES_STARTED);
        isGettingChanges = true;
        tryChangeState();
    };

    $rootScope.OnGetChangesFinish = function () {
        $rootScope.$broadcast(GET_CHANGES_FINISHED);
        isGettingChanges = false;
        tryChangeState();
    };

    $rootScope.OnUpdateCachedChangesFinished = function () {
        $rootScope.$broadcast(UPDATE_CACHED_CHANGES_FINISHED);
        isGettingChanges = false;
        tryChangeState();
    };
    ...
    service.getCollabState = function() {
        if (service.collabInfo === undefined) {
            ...
            return 'progress';
        } else if (service.collabInfo.updatingHead) {
            return 'working';
    ...
    function sortPathName(change) {
        return change.path.toLowerCase().replace('.meta', '');
    }
    ...
    service.collabInfoNotificationProxy = function (collabInfo) {
        ...
        jobPromise.then(function(isJobRunning) {
            collabInfo.updatingHead = isJobRunning;
            service.collabInfoNotificationHandler(collabInfo);
        });
    };

    service.isGettingChanges = function () {
        return proxyClient.IsGettingChanges();
    };

    service.startGetChanges = function () {
        return proxyClient.StartGetChanges();
    };

    service.tryStartGetChanges = function () {
        if (!isGettingChanges) {
            return proxyClient.StartGetChanges();
        }
    };

    service.startUpdateCachedChanges = function () {
        return proxyClient.StartUpdateCachedChanges();
    };

    service.startUpdateFileStatus = function (filePath) {
        return proxyClient.StartUpdateFileStatus(filePath);
    };
    ...
    service.ready(function (err) {
        ...
        var p5 = service.startGetChanges();

        $q.all([p1,p2,p3,p4,p5]).then(function() {
            dfd.resolve();
        });
    });
