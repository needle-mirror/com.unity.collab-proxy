...
namespace Unity.Cloud.Collaborate.Models.Providers
{
    internal class Collab : ISourceControlProvider
    {
        ...
        /// <inheritdoc />
        public event Action StartedRequestingChanges;

        /// <inheritdoc />
        public event Action FinishedRequestChanges;
        ...
        readonly List<IChangeEntry> m_Conflicts;
        readonly List<IChangeEntry> m_Changes;
        [CanBeNull]
        Timer m_ConflictsTimer;
        ...
        public Collab()
        {
            ...
            m_Conflicts = new List<IChangeEntry>();
            ...
            SetupEvents();
            UpdateConflictsInfo();
            UpdateChanges(instance.GetChangesToPublish_V2().changes);
            StartGetChanges();
        }

        protected virtual void StartGetChanges()
        {
            Bootstrap.VersionControl.StartGetChanges();
        }
        ...
        void SetupEvents()
        {
            ...
            CollabVersionControl.GetChangesStarted += OnGetChangesStarted;
            CollabVersionControl.GetChangesFinished += OnGetChangesFinished;
            CollabVersionControl.UpdateCachedChangesFinished += OnUpdateCachedChangesFinished;
        }
        ...
        /// <summary>
        /// WIP method to handle partial publish in collab and new changes function for collab v2.
        /// </summary>
        /// <param name="changes">Received changes.</param>
        /// <param name="isFiltered">Whether or not it's a partial publish.</param>
        ...
        /// <summary>
        /// Update conflicts list and start/stop timer to poll the list.
        /// </summary>
        void UpdateConflictsInfo()
        {
            if (m_ConflictCachedState)
            {
                m_ConflictsTimer = m_ConflictsTimer ?? new Timer(o => Threading.EnsureUiThread(UpdateConflicts), null, 2_000, 2_000);
                UpdateConflicts();
            }
            else
            {
                m_ConflictsTimer?.Dispose();
                m_ConflictsTimer = null;
                m_Conflicts.Clear();
                UpdatedChangeList?.Invoke();
            }
            ...
            void UpdateConflicts()
            {
                try
                {
                    var oldHash = CalculateConflictHash();
                    UpdateConflictsList(instance.GetCollabConflicts());
                    var newHash = CalculateConflictHash();

                    // If there has been a change, trigger the updated change list event.
                    if (oldHash != newHash)
                    {
                        UpdatedChangeList?.Invoke();
                    }

                    int CalculateConflictHash()
                    {
                        unchecked
                        {
                            // Calculate the hash of the list. Order matters in this hash. The starting prime seed is
                            // 487 and the previous sum of hashes is multiplied by the magic prime number 31 to ensure
                            // order is respected in the resulting hash.
                            return m_Conflicts.Aggregate(487, (sum, entry) => sum * 31 + entry.GetHashCode());
                        }
                    }
                }
                catch (Exception e)
        ...
        /// <summary>
        /// Update cached list of conflicts.
        /// </summary>
        /// <param name="conflicts">Collection of conflicted entries.</param>
        void UpdateConflictsList(IEnumerable<Change> conflicts)
        {
            m_Conflicts.Clear();
            m_Conflicts.AddRange(conflicts
                .Select(change => new ChangeEntry(change.path, change.path, ChangeEntryStatusFromCollabState(change.state),
        ...
        /// <summary>
        /// Event handler for starting get changes.
        /// </summary>
        void OnGetChangesStarted()
        {
            StartedRequestingChanges?.Invoke();
        }

        /// <summary>
        /// Event handler for finished get changes.
        /// </summary>
        void OnGetChangesFinished()
        {
            FinishedRequestChanges?.Invoke();
        }

        /// <summary>
        /// Event handler for finished updating the change cache.
        /// </summary>
        /// <param name="entriesAvailable">Whether or not there are available changes in the list.</param>
        void OnUpdateCachedChangesFinished(bool entriesAvailable)
        {
            FinishedRequestChanges?.Invoke();
        }
        ...
        void UpdateChanges(IEnumerable<ChangeItem> changes)
        {
            ...
            m_Changes.AddRange(changes.Select(change =>
                new ChangeEntry(change.Path, change.Path, ChangeEntryStatusFromCollabState(change.State),
                    false, IsCollabStateFlagSet(change.State, CollabStates.kCollabConflicted | CollabStates.kCollabPendingMerge), change))
                .Where(e => e.Status != ChangeEntryStatus.Ignored)
                .Cast<IChangeEntry>());
        }
        ...
        /// <inheritdoc />
        public bool RequestingChanges => CollabVersionControl.IsGettingChanges;

        /// <inheritdoc />
        public IReadOnlyList<IChangeEntry> ChangeList
        {
            get
            {
                // Merge conflicts and changes. Use a dict to ensure no duplicates.
                var changeDict = m_Changes.ToDictionary(e => e.Path);
                foreach (var conflict in m_Conflicts)
                {
                    var tag = conflict.Tag;
                    // Update tag object to the one from the change entry if available Changes have ChangeItem objects
                    // attached and conflicts have Change objects. Some API calls for change list items require
                    // ChangeItem objects.
                    if (changeDict.ContainsKey(conflict.Path))
                    {
                        var entry = changeDict[conflict.Path];
                        tag = entry.Tag;
                    }
                    changeDict[conflict.Path] = new ChangeEntry(conflict.Path, null, conflict.Status, conflict.Staged, conflict.Unmerged, tag);
                }
        ...
        /// <inheritdoc />
        public void RequestPublish(string message, IReadOnlyList<IChangeEntry> changeEntries)
        {
            var changeItems = changeEntries.Select(EntryToChangeItem).ToArray();
            CollabVersionControl.UpdateCachedChangesFinished += Handler;
            Bootstrap.VersionControl.StartUpdateCachedChanges();

            void Handler(bool changesAvailable)
            {
                CollabVersionControl.UpdateCachedChangesFinished -= Handler;
        ...
        {
            CollabVersionControl.GetChangesFinished += Handler;
            Bootstrap.VersionControl.StartGetChanges();
        ...
        public void RequestSync()
        {
            ...
            void Callback(int? count, IReadOnlyList<IHistoryEntry> revisions)
            {
                ...
                {
                    RequestUpdateTo(revisions[0].RevisionId);
                }
                else
                {
